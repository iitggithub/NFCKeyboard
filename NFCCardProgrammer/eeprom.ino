...
/**
 * Zeroes all memory addresses in the EEPROM memory effectively wiping it.
 * This makes sure any data since the last write is not left over.
 * 
 * @returns   void
 */
void erZeroEEPROM() {
  Serial.println("eeprom: Zero'ing EEPROM.");
    // This is the first time the code has been executed
    for (int v = 0; v <253; v++) {
      EEPROM.update(v,0);
    }
    Serial.println("eeprom: EEPROM Zeroed.");
}
...
{
  Serial.print("eeprom: Writing string: '");
  Serial.print(strToWrite);
  Serial.print("' to EEPROM memory... ");
...
/**
 * Zeroes the EEPROM and writes the randomString to EEPROM
 * 
 * @returns   bool
 */
bool erSetup() {
  erZeroEEPROM();
  erWriteStringToEEPROM(randomString);
  String erRandomString = erReadStringFromEEPROM();
  Serial.print("eeprom: EEPROM String: '");
  Serial.print(erRandomString);
  Serial.println("'.");

  if (erRandomString == randomString)
  {
    return true; 
  }
  // Something went wrong...
  Serial.println("randomString doesn't match EEPROM string!");
  return false;
}
